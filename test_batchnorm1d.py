...
def batchnorm1d_forward_func(input_tensor, weight, bias, running_mean, running_var,
                            training=True, momentum=0.1, eps=1e-5):
    """MindSpore forward calculation function"""
    # Create and configure MindSpore batch norm module
    num_features = input_tensor.shape[1]
    batch_norm_module = mint.nn.BatchNorm1d(num_features, eps=eps, momentum=momentum, affine=True)
...
def batchnorm1d_backward_func(input_tensor, weight, bias, running_mean, running_var,
                             training=True, momentum=0.1, eps=1e-5):
    """MindSpore backward propagation function"""
    def forward_func(x, w, b, rm, rv):
        return batchnorm1d_forward_func(x, w, b, rm, rv, training, momentum, eps)
    return ops.grad(forward_func, (0, 1, 2))(input_tensor, weight, bias, running_mean, running_var)


@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_basic_2d(mode):
    """
    Feature: standard forward, backward features.
    Description: test standard cases for batchnorm1d with 2D input (N, C).
    ...
    # Create and configure MindSpore batch norm module
    batch_norm_module = mint.nn.BatchNorm1d(num_features, eps=1e-5, momentum=0.1, affine=True)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_3d(mode):
    """
    Feature: standard forward, backward features.
    Description: test standard cases for batchnorm1d with 3D input (N, C, L).
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_eval_mode(mode):
    """
    Feature: eval mode support.
    Description: test batchnorm1d in evaluation mode.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_graph_mode(mode):
    """
    Feature: graph mode support for batchnorm1d.
    Description: test batchnorm1d in graph mode (KBK).
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
@pytest.mark.parametrize('dtype', [np.float32])  # Only using float32 as it's most stable
def test_batchnorm1d_dtype_coverage(mode, dtype):
    """
    Feature: dtype coverage for batchnorm1d operator.
    Description: test batchnorm1d with various dtypes.
    ...
    input_shape = (2, 3, 4)
    num_features = input_shape[1]
    input_np = generate_random_input(input_shape, dtype)
    weight_np = np.random.randn(num_features).astype(dtype)
    bias_np = np.random.randn(num_features).astype(dtype)
    running_mean_np = np.random.randn(num_features).astype(dtype)
    running_var_np = np.random.rand(num_features).astype(dtype) + 0.1  # Ensure positive variance
    # Map numpy dtypes to torch dtypes
    torch_dtype_map = {
        np.float32: torch.float32,
    }
    input_tensor = torch.tensor(input_np, dtype=torch_dtype_map[dtype])
    weight_tensor = torch.tensor(weight_np, dtype=torch_dtype_map[dtype])
    bias_tensor = torch.tensor(bias_np, dtype=torch_dtype_map[dtype])
    running_mean_tensor = torch.tensor(running_mean_np, dtype=torch_dtype_map[dtype])
    running_var_tensor = torch.tensor(running_var_np, dtype=torch_dtype_map[dtype])
    expect_output = generate_expect_forward_output(
        input_tensor, weight_tensor, bias_tensor, running_mean_tensor, running_var_tensor)
    ms_input = ms.Tensor(input_np)
    # Create and configure MindSpore batch norm module
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_with_inf_values(mode):
    """
    Feature: special value handling for batchnorm1d.
    Description: test batchnorm1d with infinite values.
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_with_nan_values(mode):
    """
    Feature: special value handling for batchnorm1d.
    Description: test batchnorm1d with NaN values.
    Expectation: results match PyTorch implementation.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_different_shapes(mode):
    """
    Feature: different input shapes support for batchnorm1d.
    Description: test batchnorm1d with various input shapes.
    Expectation: results match PyTorch implementation.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
        ...
        ms.context.set_context(mode=ms.GRAPH_MODE, jit_level="O0")
    # Test with different valid input shapes for BatchNorm1d: (N, C) and (N, C, L)
    # Note: BatchNorm1d only supports 2D and 3D inputs according to PyTorch documentation
    shapes_to_test = [(2, 3), (2, 3, 4)]  # Only valid 2D and 3D shapes
    for input_shape in shapes_to_test:
        num_features = input_shape[1]  # Channel dimension should be at index 1
        input_np = generate_random_input(input_shape)
        weight_np = np.random.randn(num_features).astype(np.float32)
        bias_np = np.random.randn(num_features).astype(np.float32)
        running_mean_np = np.random.randn(num_features).astype(np.float32)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_backward_pass(mode):
    """
    Feature: backward pass support for batchnorm1d.
    Description: test batchnorm1d backward pass.
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_non_contiguous_input(mode):
    """
    Feature: non-contiguous input support for batchnorm1d.
    Description: test batchnorm1d with non-contiguous input tensors.
    Expectation: results match PyTorch implementation.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
        ...
        ms.context.set_context(mode=ms.GRAPH_MODE, jit_level="O0")
    input_shape = (4, 6, 8)  # Make it larger to allow slicing
    num_features = input_shape[1]
    input_np = generate_random_input(input_shape)
    # Create non-contiguous array by taking every other element
    input_np_non_contiguous = input_np[:, :, ::2]  # Take every second element in last dim
    weight_np = np.random.randn(num_features).astype(np.float32)
    bias_np = np.random.randn(num_features).astype(np.float32)
    running_mean_np = np.random.randn(num_features).astype(np.float32)
    running_var_np = np.random.rand(num_features).astype(np.float32) + 0.1  # Ensure positive variance
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_backward_eval_pass(mode):
    """
    Feature: backward pass support for batchnorm1d in eval mode.
    Description: test batchnorm1d backward pass in evaluation mode.
    Expectation: gradients match PyTorch implementation.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm1d_broadcast_shapes(mode):
    """
    Feature: broadcast support for batchnorm1d.
    Description: test batchnorm1d with broadcasting scenarios.
    Expectation: results match PyTorch implementation.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
    ...
    # Standard case: (batch_size, channels, length)
    input_shape = (4, 3, 5)  # 4 samples, 3 channels, sequence length 5
