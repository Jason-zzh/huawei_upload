...
namespace op_plugin {
namespace aten_op {
extern "C" int FmodTensor(int nparam, void **params, int *ndims, int64_t **shapes, const char **dtypes, void *stream,
                          void *extra) {
  ...
  at::fmod_out(at_output, at_input1, at_input2);

  // Ensure output tensor properties match input tensors exactly
  at_output = at_output.to(at_input1.options());
