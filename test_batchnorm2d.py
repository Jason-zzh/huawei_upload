...
def batchnorm2d_forward_func(input_tensor, weight, bias, running_mean, running_var,
                            training=True, momentum=0.1, eps=1e-5):
    """MindSpore forward calculation function"""
    # Create and configure MindSpore batch norm module
    num_features = input_tensor.shape[1]
    batch_norm_module = mint.nn.BatchNorm2d(num_features, eps=eps, momentum=momentum, affine=True)
...
def batchnorm2d_backward_func(input_tensor, weight, bias, running_mean, running_var,
                             training=True, momentum=0.1, eps=1e-5):
    """MindSpore backward propagation function"""
    def forward_func(x, w, b, rm, rv):
        return batchnorm2d_forward_func(x, w, b, rm, rv, training, momentum, eps)
    return ops.grad(forward_func, (0, 1, 2))(input_tensor, weight, bias, running_mean, running_var)


@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative'])
def test_batchnorm2d_basic_4d(mode):
    """
    Feature: standard forward, backward features.
    Description: test standard cases for batchnorm2d with 4D input (N, C, H, W).
    ...
    # Create and configure MindSpore batch norm module
    batch_norm_module = mint.nn.BatchNorm2d(num_features, eps=1e-5, momentum=0.1, affine=True)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative'])
def test_batchnorm2d_different_shapes(mode):
    """
    Feature: standard forward, backward features.
    Description: test batchnorm2d with different input shapes.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative'])
def test_batchnorm2d_eval_mode(mode):
    """
    Feature: eval mode support.
    Description: test batchnorm2d in evaluation mode.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
def test_batchnorm2d_graph_mode(mode):
    """
    Feature: graph mode support for batchnorm2d.
    Description: test batchnorm2d in graph mode (KBK).
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
    ...
    batch_norm_module.set_train(True)
    output = batch_norm_module(ms_input)
    allclose_nparray(expect_output.detach().numpy(), output.asnumpy(), rtol=1e-4, atol=1e-5)


@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])

def test_batchnorm2d_backward_pass(mode):
    """
    Feature: backward pass support for batchnorm2d.
    Description: test batchnorm2d backward pass.
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_backward_eval_pass(mode):
    """
    Feature: backward pass support for batchnorm2d in eval mode.
    Description: test batchnorm2d backward pass in evaluation mode.
    Expectation: gradients match PyTorch implementation.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_special_values_inf(mode):
    """
    Feature: special value handling (infinity).
    Description: test batchnorm2d with infinity values in input.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_special_values_nan(mode):
    """
    Feature: special value handling (NaN).
    Description: test batchnorm2d with NaN values in input.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_special_values_zero(mode):
    """
    Feature: special value handling (zero).
    Description: test batchnorm2d with zero values in input.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_special_values_large(mode):
    """
    Feature: special value handling (large values).
    Description: test batchnorm2d with large values in input.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_special_values_small(mode):
    """
    Feature: special value handling (small values).
    Description: test batchnorm2d with small values in input.
    Expectation: expect correct result.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_non_contiguous_input(mode):
    """
    Feature: non-contiguous input support for batchnorm2d.
    Description: test batchnorm2d with non-contiguous input tensors.
    ...
    if mode == 'pynative':
        ...
        ms.context.set_context(mode=ms.GRAPH_MODE, jit_level="O0")
    input_shape = (4, 6, 8, 8)  # Make it larger to allow slicing
    num_features = input_shape[1]
    input_np = generate_random_input(input_shape)
    # Create non-contiguous array by taking every other element
    input_np_non_contiguous = input_np[:, :, ::2, :]  # Take every second element in height dimension
    weight_np = np.random.randn(num_features).astype(np.float32)
    bias_np = np.random.randn(num_features).astype(np.float32)
    running_mean_np = np.random.randn(num_features).astype(np.float32)
    running_var_np = np.random.rand(num_features).astype(np.float32) + 0.1  # Ensure positive variance
...
@arg_mark(plat_marks=['cpu_linux'], level_mark='level0', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative', 'KBK'])
def test_batchnorm2d_broadcast_shapes(mode):
    """
    Feature: broadcast support for batchnorm2d.
    Description: test batchnorm2d with broadcasting scenarios.
    Expectation: results match PyTorch implementation.
    """
    if mode == 'pynative':
        ms.context.set_context(mode=ms.PYNATIVE_MODE)
    ...
    input_shape = (4, 3, 16, 16)  # 4 samples, 3 channels, 16x16 images
