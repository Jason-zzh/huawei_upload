...
namespace op_plugin {
namespace aten_op {
extern "C" int Addmv(int nparam, void **params, int *ndims, int64_t **shapes, const char **dtypes, void *stream,
                     void *extra) {
  ...
  auto mat = tensors[1];    // mat tensor
  auto vec = tensors[2];    // vec tensor
  auto output = tensors[nparam - 1];  // output tensor

  at::addmv_out(output, input, mat, vec, beta, alpha);
  return 0;
}
}  // namespace aten_op
}  // namespace op_plugin
